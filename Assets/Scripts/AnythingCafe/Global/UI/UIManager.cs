...
public class UIManager : PersistentSingleton<UIManager>, IInitializable
{
    ...
    [SerializeField] private SerializableDictionary<string, SerializableKeyValuePair<int, ReactiveComponent>> _editableComponents; // 可在Inspector中编辑的控件

    [SerializeField] private GraphicRaycaster _graphicRaycaster; // 用于处理UI事件的Raycaster

    private readonly Dictionary<int, List<ReactiveComponent>> _allReactiveComponents; // 场景中所有的ClosableUI,int为UI层级
    private readonly LinkedList<ReactiveComponent> _activeComponents;  // 正在显示的UI,类似栈的机制
    private readonly List<ReactiveComponent> _closingComponents; // 正在关闭的UI队列,类似GC的机制
    ...
    public void ResetAllComponents()
    {
        ...
        // 重新注册所有可编辑控件
        foreach (var component in _editableComponents.Values)
        {
